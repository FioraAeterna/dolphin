...
void XEmitter::WriteAVXOp(int size, u16 sseOp, bool packed, X64Reg regOp1, X64Reg regOp2, OpArg arg, int extrabytes)
{
	...
	int mmmmm = (sseOp >> 8) == 0x3A ? 3 : (sseOp >> 8) == 0x38 ? 2 : 1;
	arg.WriteVex(this, regOp1, regOp2, size == 256, (packed << 1) | (size == 64), mmmmm);
	Write8(sseOp & 0xFF);
...
void XEmitter::WriteVEXOp(int size, u8 opPrefix, u16 op, X64Reg regOp1, X64Reg regOp2, OpArg arg, int extrabytes)
{
	...
	int mmmmm = (op >> 8) == 0x3A ? 3 : (op >> 8) == 0x38 ? 2 : 1;
	int pp = opPrefix == 0x66 ? 1 : opPrefix == 0xF3 ? 2 : opPrefix == 0xF2 ? 3 : 0;
...
void XEmitter::RORX(int bits, X64Reg regOp, OpArg arg, OpArg rotate)
{
	if (!rotate.IsImm())
	{
		_assert_msg_(DYNA_REC, 0, "RORX - rotate must be immediate");
	}
	WriteBMI2Op(bits, 0xF2, 0x3AF0, regOp, INVALID_REG, arg, 1);
	Write8((u8)rotate.offset);
}
...
void XEmitter::MULX(int bits, X64Reg regOp1, X64Reg regOp2, OpArg arg) {WriteBMI2Op(bits, 0xF2, 0x38F6, regOp1, regOp2, arg);}
